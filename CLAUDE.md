...
src/
  ...
  services/
    api/ + api.js      HTTP calls (api/ split by domain; api.js re-exports for legacy call sites)
    socket.ts          Socket.IO — emitLocation, start/stopTracking
    backendStatus.js   Health poller + NetInfo — three-way online/degraded/offline state
    ...
    backgroundLocation.ts  # TaskManager background location task (registered from App.js
                           # at module scope — the OS can launch the process headless, so this
                           # registration must never move into a screen)
  lib/                 queryKeys, errors
  helpers/             formatters, geo (haversine), locationUtils (shouldEmit throttle), tripProgress
...
- **Background tracking's `TaskManager.defineTask` stays at module scope.** It's registered from
  `App.js` because the OS can launch the process headless — moving it into a screen breaks it silently.
