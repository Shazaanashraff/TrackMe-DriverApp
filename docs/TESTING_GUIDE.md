...
| EnrollmentKeyCard + useMyEnrollmentKeyQuery | unit | features/profile/__tests__/EnrollmentKeyCard.test.tsx + hooks/auth/__tests__/auth.test.ts | shows the key; copies it and returns the button to "Copy" after the confirmation; shares a message containing the key and survives Share being unavailable or dismissed; wording follows `isPrivate`; a rotation drops a stale "Copied"; error offers a retry rather than a blank card; the query refetches on mount because a rotation silently voids the cached key | the key card's actions change, or the key stops being manager-rotatable |
