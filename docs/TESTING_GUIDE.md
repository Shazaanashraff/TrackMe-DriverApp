...
| VehicleCard (renamed from AssignedBusCard) | unit | features/dashboard/__tests__/VehicleCard.test.tsx | shows the vehicle name only — no internal ID, no seat count; route name as the sub-line when present (absent, not blank, when there is none); renders the `bus` Ionicons glyph rather than the missing-icon "?" that `name="vehicle"` produced; no-bus EmptyState + Add-my-bus action | STYLEGUIDE §6.2 VehicleCard spec changes, or seat/ID display returns |
