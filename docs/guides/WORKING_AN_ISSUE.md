# Working a GitHub issue — driver-app
...
Issues live on [`Shazaanashraff/TrackMe-DriverApp`](https://github.com/Shazaanashraff/TrackMe-DriverApp)
...
```bash
gh issue view <N> --repo Shazaanashraff/TrackMe-DriverApp
```
...
npm run typecheck
npm run lint
npm run test:e2e:dry   # maestro --dry-run; run test:e2e for real on a device/sim if the change
                        # touches a user-facing flow the dry-run can't validate
```
...
- That checklist is a **floor, not a ceiling** — if the change touches something it didn't
  anticipate, add the test per [`ADDING_A_TEST.md`](ADDING_A_TEST.md) and run it too.
- **Touches GPS broadcast, the socket, or duty state?** Read
  [`../LOCATION_TRACKING.md`](../LOCATION_TRACKING.md) first — this is the one path where a
  regression is invisible from inside this app and only shows up as a stale bus for passengers.
  Changing a socket event also means updating
...
- [x] npm run typecheck
- [x] npm run lint
- [x] <e2e / anything else the issue or ADDING_A_TEST.md called for>
"
```
...
- [ ] Socket contract changes reflected in `backend/docs/modules/REALTIME.md`.
