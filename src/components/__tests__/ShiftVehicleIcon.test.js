...
import ShiftVehicleIcon from '../ShiftVehicleIcon';
import { theme } from '../../theme';

describe('ShiftVehicleIcon', () => {
  it('renders without crashing at the given size', () => {
    const { toJSON } = render(<ShiftVehicleIcon size={56} />);
    const tree = toJSON();
  ...
  it('accepts custom body/detail colors', () => {
    const { toJSON } = render(<ShiftVehicleIcon bodyColor="#123456" detailColor="#abcdef" />);
    const xml = toJSON().props.xml;
